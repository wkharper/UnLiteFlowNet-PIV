...
def test_estimate(flow_type,fps,arrow_density):
    ...
    images = []
    if isVideo:
        print("Collecting Flowiz Image Flows...")
        images = []
        for f in  sorted(glob.iglob(f'{result_path}/frame*.png')):
            images.append(imageio.imread(f))

        print("Saving Flowiz Video with " + str(len(images)) + " images...This may take a while. Please wait.")
        imageio.mimsave(result_path + '/movie.gif', images, format='GIF', duration=1000/int(fps))

        print("Collecting Raw Flowiz Image Flows")
        raw_images = []

        for f in  sorted(glob.iglob(f'{result_path}/raw_frame*.png')):
            raw_images.append(imageio.imread(f))

        print("Saving Raw Flowiz Video with " + str(len(raw_images)) + " images...This may take a while. Please wait.")
        imageio.mimsave(result_path + '/raw_movie.gif', raw_images, format='GIF', duration=1000/int(fps))

        print("Collecting Custom Color Image Flows")
        cv_images = []

        for f in  sorted(glob.iglob(f'{result_path}/cv_frame*.png')):
            cv_images.append(imageio.imread(f))

        print("Saving Custom Color Video with " + str(len(cv_images)) + " images...This may take a while. Please wait.")
        imageio.mimsave(result_path + '/cv_movie.gif', cv_images, format='GIF', duration=1000/int(fps))
